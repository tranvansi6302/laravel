...
@section('content')
    ...
    <section class="d-flex justify-content-center mt-5">
        <div style="width: 500px;">
            <h2 class="text-secondary text-uppercase mb-3">Sửa người dùng
                <i class="fa-sharp fa-solid fa-users-medical ms-1"></i>
            </h2>
            <form id="validation" class="needs-validation" action="{{ route('users.update') }}" method="POST" novalidate>
                <div class="mt-3">
                    <label for="fullname"
                    ...
                    <div class="input-group has-validation">
                        <input value="{{ old('fullname') ?? ($userDetail->fullname ?? false) }}" name="fullname"
                            type="text" class="form-control py-2 shadow @error('fullname') is-invalid @enderror"
                            id="fullname" aria-describedby="inputGroupPrepend" placeholder="Nhập họ và tên...">
                        @error('fullname')
                ...
                <div class="mt-2">
                    ...
                    <div class="input-group has-validation">
                        <input value="{{ old('email') ?? ($userDetail->email ?? false) }}" name="email" type="text"
                        ...
                        <select name="group_id" class="form-select shadow @error('group_id') is-invalid @enderror">
                            ...
                            @if (!empty($allGroup))
                                @foreach ($allGroup as $group)
                                    <option
                                        {{ !empty($userDetail) && (old('group_id') == $group->id || $userDetail->group_id == $group->id) ? 'selected' : false }}
                                        value="{{ $group->id }}">
                                        {{ $group->name }}</option>
                                @endforeach
                            @endif
                ...
                <div class="mt-2">
                    ...
                    <select name="status" class="form-select shadow">
                        <option
                            {{ !empty($userDetail) && (old('status') == 0 || $userDetail->status == 0) ? 'selected' : false }}
                            value="0">Chưa kích hoạt</option>
                        <option
                            {{ !empty($userDetail) && (old('status') == 0 || $userDetail->status == 1) ? 'selected' : false }}
